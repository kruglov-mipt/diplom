...
class Tag:
    ...
    def process_query_adjust(self, frame):
        ...
        self._active_session = qadjust.session
        ...
        brand_new_q = 0
        if self._q > 1:
            brand_new_q = self._q + qadjust.upDn.eval()
        else:
            brand_new_q = self._q

        self._slot_counter = np.random.randint(0, pow(2, brand_new_q))
...
class Generator:
    ...
    velocity = 60.0 / 3.6  # meters per second
    direction = None  # should be a 3-dim np.array
    travel_distance = 40.0  # meters
    epc_bitlen = 96
    tid_bitlen = 64
    epc_prefix = 'AAAA'
    tid_prefix = 'AAAA'
    ...
    inversed_antenna_direction = False
    ...
    def create_tag(self, model):
        ...
        # Antenna and power parameters
        if self.inversed_antenna_direction:
            tag.antenna.direction_theta = -1 * tag.direction
        else:
            tag.antenna.direction_theta = tag.direction
